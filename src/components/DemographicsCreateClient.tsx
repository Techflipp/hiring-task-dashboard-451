...
const CreateDemographicConfig = ({ moode }: { moode: Moode }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await api[moode === "edit" ? "put" : "post"](
                `/demographics/config${moode === "edit" ? `/${camera?.demographics_config.id}` : ""}`,
                {
                    ...formData,
    ...
    const InputField: React.FC<InputFieldProps> = ({ label, field, type = "number", step, min, max, required = false }) => (
        <div className="space-y-2">
            ...
            <input
                ...
                className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent transition-colors ${errors[field] ? "border-red-500 focus:ring-red-500" : ""
                    }`}
                    ...
                    <div className="flex items-center justify-between h-16">
                        <div className="flex items-center space-x-3">
                            ...
                            <h1 className="text-xl font-semibold text-gray-900">
                                {moode === "edit" ? "Edit Demographic Configuration" : "Create Demographic Configuration"}
                            </h1>
                        </div>
                    </div>
                </div>
                        ...
                        >
                            {isSubmitting ? (
                                <>
                                    ...
                                    <span>{moode === "edit" ? "Updating..." : "Creating..."}</span>
                                </>
                            ) : (
                                <>
                                    <Save className="w-4 h-4" />
                                    <span>{moode === "edit" ? "Update Configuration" : "Create Configuration"}</span>
                                </>
                            )}
                        </button>
                    </div>
