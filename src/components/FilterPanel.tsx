...
export default function FilterPanel({
  ...
  return (
    <div className="fixed inset-0 shadow z-50 flex justify-end bg-black/50" onClick={onClose}>
      <motion.div
      ...
      >
        ...
        <div className="space-y-6">
          {Object.entries(filterOptions).map(([key, values]) => (
            <div key={key}>
              <h3 className="text-lg font-medium text-gray-700 mb-3 capitalize">{key}</h3>
              <div className={`grid ${values.length > 3 ? 'grid-cols-2' : 'flex flex-wrap'} gap-2`}>
                {values.map((value) => (
                  <button
                    key={value}
