...
import CameraDemographicsConfigForm from "@/components/CameraDemographicsConfigForm";
...
export default function CameraDetailPage({ serverData }: { serverData?: any }) {
  const { id } = useParams();
  const { data, isLoading, error } = useQuery({
    queryKey: ["camera", id],
    queryFn: async () => {
      const res = await getCameraDetails(serverData?.id);
      return res;
    },
    enabled: !!id,
